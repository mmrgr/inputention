...
Inputention 是一个 Codex skill，用来帮助用户更顺畅地和 AI 对话，尤其适合打字较慢、懒得反复组织长句，或担心 AI 无法理解自己而产生遣词造句压力的场景。它可以把“我接下来可能想问什么”或“我只输入了一个模糊关键词”转化成一组按可能性排序的自然语言候选项。用户只需要回复序号，必要时补充几个字段，助手就能拼合出完整意图并继续回答。
...
- **持续预测**：一旦启用，语义清楚的每次回答后都追加下一步输入预测，直到用户明确停止。
- **默认预测 5 条**：比 9 条更轻，不容易打扰正常对话。
- **可调整预测数量**：只发送 `？？3` 或 `??3`，即可把后续预测数量改为 3 条。
- **预测下一步输入**：根据当前对话生成最可能的后续输入。
- **先补全再回答**：把关键词、半句话、指代不明内容转成 3-9 个完整意图候选，等待用户下一轮用序号选择或补充后再回答。
- **支持序号回复**：识别 `2`、`2；A；B`、`我选2`、`I choose 2`、`use #2` 等形式。
- **自动填充待补信息**：按顺序填充 `【报错内容】`、`【目标风格】`、`[error message]` 等待补信息。
- **减轻措辞压力**：用户可以选择或轻微改动候选意图，不必总担心“我这样说 AI 会不会理解错”。
...
2. 用户打字慢，希望用更少操作继续推进对话。
3. 用户担心 AI 不理解自己，于是在发送前反复斟酌措辞，增加了心理负担。

Inputention 通过“候选项 + 序号补全”解决这些问题：
...
- 输出可直接复制或通过序号选择的完整句子。
- 默认输出 5 条预测。
- 如果用户通过 `？？3` / `??3` 修改过数量，就严格按当前数量输出。
- 预测必须包含开放式范式；只要后续问题可能依赖用户个人信息，至少 `min(3, 当前预测数量)` 条要带具体待补信息。

### 预测数量设置

预测数量只影响“清楚回答后的下一步输入预测”，不影响意图补全。意图补全仍然是 3-9 个候选。
...
- `？？ 3` 和 `?? 3` 也可以识别。
- 这类设置消息只更新数量并简短确认。
- 设置消息本身不触发回答、不触发意图补全、不追加预测。

### 2. 意图补全

当用户输入还不是完整、可确定的请求时，使用这个模式。此时不直接回答，而是先给出 3-9 个最可能的完整意图候选，等待用户下一轮选择或补充。
...
- 尽量保留用户原始关键词。
- 生成最小但够用的高质量候选，最少 3 个，最多 9 个。
- 用具名待补信息表示缺失事实，而不是替用户编造。
- 只要能生成合理候选，就避免只问“你是什么意思？”。
- 如果用户没有选择候选，而是直接发来一条新消息，就把新消息当作当前请求处理，不再纠结上一轮候选。
- 补全候选列表后不直接回答，也不追加预测块；等用户下一轮选择或补充后再回答。
...
- 缺少必要内容时只追问缺失项。
- 拼合完整请求后继续完成任务。
- 如果 Inputention 仍处于启用状态，回答后继续追加当前预测数量的下一步输入预测。
...
触发逻辑主要写在 `SKILL.md` 的 frontmatter `description` 和正文的模式选择规则中。当前项目不再包含 `agents/openai.yaml`。
...
- 如果 `?` 后面的文字已经比较清楚，应先回答该问题，然后追加当前预测数量的下一步输入预测。
...
这表示后续预测块改为 3 条。数字范围是 1-9。

注意：`?3`、`???3`、`？？三`、`？？3 请继续` 都不会被当作预测数量设置。
...
- Inputention 启用后，如果用户表达清楚，就直接回答并追加当前预测数量的下一步输入预测。
- 用户表达不清或意图无法完整确定时，给出 3-9 个排序后的意图候选，并等待用户选择或补充。
- 用户选择候选后，补全真实意图并完成任务。
...
- `tests/checklist.md`：人工测试清单，覆盖触发、预测数量设置、意图补全、序号补全和停止行为。
- `tests/cases.yaml`：机器可读测试场景，可供未来测试脚本使用，也可人工对照检查。
