...
- **持续预测**：一旦启用，语义清楚的每次回答后都追加 9 条下一步输入预测，直到用户明确停止。
...
当用户明确要求预测后续输入，或 Inputention 已启用且刚完成一次语义清楚的回答时，使用这个模式。
...
- 按可能性从高到低排序。
- 输出可直接复制或通过序号选择的完整句子。
- 在预测模式中始终输出 9 条，不少于 9 条。
- 9 条预测可以混合完整句子和带占位符的开放式范式。
...
- 如果用户没有选择候选，而是直接发来一条新消息，就把新消息当作当前请求处理，不再纠结上一轮候选。
...
- 缺少必要内容时只追问缺失项。
- 拼合完整请求后继续完成任务。
- 如果 Inputention 仍处于启用状态，回答后继续追加 9 条下一步输入预测。
...
`?` 或 `？` 触发后，Inputention 会在后续对话中持续生效，直到用户明确说“停止预测”“不需要这个功能”“不要再预测”等。
...
- 如果 `?` 后面的文字已经比较清楚，应先回答该问题，然后追加 9 条下一步输入预测。
- 如果用户没有选择上一轮预测或澄清候选，而是直接发来新输入，就按新输入继续处理，不再反复要求选择旧选项。
...
- Inputention 未启用且用户表达清楚时，直接回答。
- 用户以 `?` 或 `？` 开头时，启用持续 Inputention 行为。
- Inputention 启用后，如果用户表达清楚，就直接回答并追加 9 条下一步输入预测。
- 用户表达不清且直接回答容易答偏时，给出排序后的意图候选。
- 用户选择候选后，补全真实意图并完成任务。
- 用户不选候选而直接发来新输入时，处理新输入，不再卡在旧候选上。
